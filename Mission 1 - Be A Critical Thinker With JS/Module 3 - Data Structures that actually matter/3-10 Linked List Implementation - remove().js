// * Cpu scheduler-> round robing algorith, OPERATINNG SYSTEM (circular linked list). LIST DUCED CACHE(DOUBLY LINKED LIST)
...
class LinkedList {
    ...
    // ! Worst Case -> 0(n)
    remove(index) {
        // ! When first node is being removed
        if (index === 0) {
            // this.head = nodeToRemove.next //! Has to run whole method to get nodeToRemove
            this.head = this.head.next

            // ! If whole linkedlist is that only toberemoved node
            if(this.length === 1){
                this.tail = null
                // this.head = null //It's being null in upperline anyway
            }

            this.length--;
            return 'Node removed' //! Has to return here, so it stops here and doesnt run whole method.
        }

        //! to remove some index in middle or any
        const prevNode = this._traverseToIndex(index - 1)
        const nodeToRemove = this._traverseToIndex(index)

        //! When last node is removed. so tail has to change
        if (nodeToRemove.next === null) {
            this.tail = prevNode
        }

        prevNode.next = nodeToRemove.next //! Adding the removing index's prev node's next to removing index's next node
        this.length--;
    }

    //! '_' -> private helper method
    // !O(n) -> for this many other mehtod O(n). cz using this in others 
...
linkedList.print() //! 10->20->30->null

linkedList.prepend(1)
linkedList.prepend(2)
linkedList.prepend(3)
console.log('After prepending data->') //! 3->2->1->10->20->30->null
linkedList.print()

linkedList.insert(2, 500) //! Here the node before is supposed to be 2
console.log('After inserting a node at an index->')
linkedList.print() //! 3->2->500->1->10->20->30->null


linkedList.remove(2)
console.log('After removing 2nd indexed data->')
linkedList.print()